...
rm_except_chars = re.compile(u'[^\w\s\d…\-–\./_,\(\)$%%“”\"\'?!;:@#^&*\+=<>\[\]\{\}²³áÁàÀãÃảẢạẠăĂắẮằẰẳẲặẶẵẴâÂấẤầẦẩẨậẬẫẪđĐéÉèÈẻẺẽẼẹẸêÊếẾềỀễỄểỂệỆíÍìÌỉỈĩĨịỊóÓòÒỏỎõÕọỌôÔốỐồỒổỔỗỖộỘơƠớỚờỜởỞỡỠợỢúÚùÙủỦũŨụỤưƯứỨừỪửỬữỮựỰýÝỳỲỷỶỹỸỵỴ]')
...
def review_to_words(review, filename):
    ...
    # words2 = [word for word in meaningful_words if word not in stopwords.words('english')]
...
def load_data(filename, dict):
    ...
    with open(filename, 'r') as f,open(dict, "w") as f2:
        for line in f:
            ...
            question = review_to_words(question,'data/dict1')
            # question = rm_except_chars.sub(u'',question)
...
if __name__ == "__main__":
    ...
    train = load_data('data/train_3000.label','data/dict1')
    test = load_data('data/TREC_10.label','data/dict1')
    ...
    clf = load_model('model/uni_big.pkl')

...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'data/train_5500.label':
            joblib.dump(train, 'model/train_tfidf1.pkl')
        else:
            joblib.dump(train, 'model/test_tfidf1.pkl')
    return train


def svm():
    train = load_model('model/train_tfidf1.pkl')
    if train is None:
        train = load_data('data/train_5500.label')

    vectorizer = load_model('model/vectorizer_tfidf1.pkl')
    if vectorizer == None:
        vectorizer = TfidfVectorizer(ngram_range=(1, 1), max_df=0.7, min_df=2, max_features=1000)
    test = load_model('model/test_tfidf1.pkl')
    if test is None:
    ...
    joblib.dump(vectorizer, 'model/vectorizer_tfidf1.pkl')
    ...
    clf = load_model('model/tfidf1.pkl')
    if clf is None:
        t0 = time.time()
        clf = SVC(kernel='rbf', C=1000, class_weight='balanced')
        clf.fit(X_train, y_train)
        joblib.dump(clf, 'model/tfidf1.pkl')
    ...
    print "-----------------------"
    clf2 = load_model('model/tfidf_fine.pkl')
    if clf2 is None:
        t2 = time.time()
        clf2 = SVC(kernel='rbf', C=1000, class_weight='balanced')
        clf2.fit(X_train, y_train2)
        joblib.dump(clf, 'model/tfidf_fine.pkl')
